...
export const Third: React.FC<any> = ({ coach, seats, direction, id_route }) => {
  const planSeats = [
  ...
  const planSeatsWitchPrice = planSeats.map(item => {
    if (item.number % 2 === 0 && item.number < 33) {
      return { ...item, price: coach.top_price }
    } else if (item.number > 32) {
      return { ...item, price: coach.side_price }
    } else if (item.number % 2 !== 0 && item.number < 33) {
